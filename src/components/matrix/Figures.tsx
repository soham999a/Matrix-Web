...
function grow(x: number, y: number, len: number, angle: number, depth: number, out: Branch[]) {
...
export function RecursiveIntelligence({ className }: Props) {
  ...
  return (
    <Frame
      className={className}
      label="A recursive branching system built on golden-ratio proportions"
    >
  ...
  return (
    <Frame className={className} label="Standing waves and interference forming ordered structure">
      <g className="cx" stroke="var(--color-gold)" strokeOpacity="0.45" strokeWidth="0.4">
        ...
        {[40, 80, 120, 160].map((r) => (
          <circle key={r} cx={cx} cy={cy} r={r} strokeOpacity="0.3" />
        ))}
...
export function RelationalIntelligence({ className }: Props) {
  return (
    <Frame
      className={className}
      label="The Orion constellation — meaning exists between the stars, not within them"
    >
...
export function Transformation({ className }: Props) {
  ...
  return (
    <Frame
      className={className}
      label="A beam refracted through a prism into several coherent outputs"
    >
      <g className="cx" stroke="var(--color-gold)" strokeOpacity="0.45" strokeWidth="0.4">
      ...
      <g className="form fig-breathe">
        ...
        {/* internal path */}
        <path
          d={`M${hit[0]} ${hit[1]} L${exit[0]} ${exit[1]}`}
          stroke="var(--color-gold)"
          strokeWidth="0.6"
          strokeOpacity="0.7"
        />
          ...
          {fans.map((d, i) => {
            ...
            return (
              <line
                ...
                x1={exit[0]}
                y1={exit[1]}
                x2={exit[0] + Math.cos(a) * len}
                y2={exit[1] + Math.sin(a) * len}
    ...
    <Frame className={className} label="A folded structure opening beneath an observatory geometry">
      ...
      <g className="form fig-breathe">
        ...
        <g className="fig-fold" stroke="currentColor">
          <path
            d={`M${cx} ${cy - 112} L${cx - 112} ${cy} L${cx} ${cy} Z`}
            strokeWidth="0.6"
            strokeOpacity="0.55"
          />
          <path
          ...
          />
          <path
            d={`M${cx - 112} ${cy} L${cx - 56} ${cy - 78} L${cx} ${cy} Z`}
            strokeWidth="0.5"
            strokeOpacity="0.3"
          />
          <path
