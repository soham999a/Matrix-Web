  ...
  return (
    <aside className="live-pill fixed bottom-5 right-5 sm:bottom-8 sm:right-8 z-40">
      <div className="group flex items-center gap-4 border border-gold/60 bg-[#F2EFE9] px-5 py-3 shadow-2xl hover:border-gold transition-colors duration-500">
        ...
        <span className="hidden sm:block h-4 w-px bg-ink/15" />
        <nav className="hidden sm:flex items-center gap-3">
          {liveProducts.map((p) => (
            <a
              ...
              className="font-mono text-[10px] tracking-[0.2em] uppercase text-ink/70 hover:text-ink transition-colors duration-300 whitespace-nowrap"
            >
              {p.t} ↗
            </a>
          ))}
        ...
        <Link
          href="/products"
          className="sm:hidden font-mono text-[10px] tracking-[0.2em] uppercase text-ink/70 hover:text-ink transition-colors duration-300 whitespace-nowrap"
        >
          4 live ↗
        </Link>
      </div>
